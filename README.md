...
- Markdown representations: canonical page URLs return generated Markdown when an agent requests `Accept: text/markdown`, and `/path.md` returns the exact same representation for clients that cannot set headers. HTML remains the default on canonical URLs, quality values and HTTP 406 are handled, and optional files under `src/content/markdown` can override individual paths. See `docs/recipes/markdown-negotiation.md`.
