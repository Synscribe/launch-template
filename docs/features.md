...
| Markdown content negotiation                 | Keep                      | Canonical pages generate Markdown from rendered `<main>` content; optional pathname-based file overrides are supported     | P0                       | `src/proxy.ts`, `src/app/api/agent-markdown`, `docs/recipes/markdown-negotiation.md`             |
| Explicit `.md` sibling URLs                  | Keep                      | `/path.md` uses the exact generated or overridden Markdown representation for `/path`; HTML stays canonical                | P2                       | `src/proxy.ts`, `src/lib/markdown-routing.ts`, `docs/recipes/markdown-negotiation.md`            |
