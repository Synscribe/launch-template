...
4. Choose one supported `id` for each capability. The ID selects its route-owned visual from `src/app/uses/[slug]/_components/feature-visuals.tsx`; the JSON does not describe React components, layout, or visual props.
...
6. Run `pnpm build`. `generateStaticParams` should list the new `/uses/<slug>` route.
7. Confirm the route is linked from the grouped `/uses` hub and appears in `/sitemap.xml`.
...
- Route-owned feature visuals: `src/app/uses/[slug]/_components/feature-visuals.tsx`
